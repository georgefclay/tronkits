...
// Randomness comes from crypto.randomInt (a CSPRNG), matching the Python
// generator in python/passphrase_generator.py, which uses `secrets`.
...
function randomSep(exactMode) {
  ...
  const r = crypto.randomInt(100); // 0-99
  if (r < 70) return pick(DIGITS_1.concat(SYMS_1));
  if (r < 85) return pick(DIGITS_2.concat(SYMS_2));
  return pick(MIXED_2);
}
